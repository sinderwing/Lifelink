 ...
 */
public class LobbyJoiningBroadcastReceiver extends BroadcastReceiver {

    private WifiP2pManager wifiP2pManager;
    private WifiP2pManager.Channel channel;
    private LobbyJoining activity;

    /**
     * Create a LobbyJoiningBroadcasrReceiver object.
     ...
     */
    public LobbyJoiningBroadcastReceiver(WifiP2pManager wifiP2pManagermanager,
                                    WifiP2pManager.Channel channel, LobbyJoining activity) {
        super();
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (WifiP2pManager.WIFI_P2P_STATE_CHANGED_ACTION.equals(action)) {
            ...
            Log.d(LobbyJoining.TAG, "P2P state changed: " + state);

        } else if (WifiP2pManager.WIFI_P2P_PEERS_CHANGED_ACTION.equals(action)) {

            if (wifiP2pManager != null) {
                wifiP2pManager.requestPeers(channel, (WifiP2pManager.PeerListListener)
                        activity.getFragmentManager().findFragmentById(R.id.joining_lobby_hosting_list));
            }
